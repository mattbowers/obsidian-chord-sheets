				...
				if (node.type.name.contains("HyperMD-codeblock-begin")) {
					...
					if (chordBlockStartMatch) {
						if (chordBlockStartMatch[2]) {
							if (chordBlockStartMatch[2] === "continued") {
								// `chords-continued` is a fork extension handled by the reading-mode
								// post processor; treat it as the default instrument here.
			...
			chordDecos.push(
				Decoration
					.mark({ class: "chord-sheet-quoted",
						attributes: { type: token.quoteType }})
